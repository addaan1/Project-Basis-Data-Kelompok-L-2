                    ...
                    <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="id_petani" class="form-select @error('id_petani') is-invalid @enderror" required>
                                ...
                                @foreach($petanis as $petani)
                                    <option value="{{ $petani->id_user }}" {{ old('id_petani') == $petani->id_user ? 'selected' : '' }}>{{ $petani->nama }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="text" name="varietas" class="form-control @error('varietas') is-invalid @enderror" value="{{ old('varietas') }}" required>
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="kualitas" class="form-select @error('kualitas') is-invalid @enderror" required>
                                    ...
                                    <option value="Medium" {{ old('kualitas') == 'Medium' ? 'selected' : '' }}>Medium</option>
                                    <option value="Standard" {{ old('kualitas') == 'Standard' ? 'selected' : '' }}>Standard</option>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                ...
                                <input type="number" name="harga_per_kg" class="form-control @error('harga_per_kg') is-invalid @enderror" value="{{ old('harga_per_kg') }}" min="0" required>
                            </div>
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Stock (Kg)</label>
                                <input type="number" name="stok_kg" class="form-control @error('stok_kg') is-invalid @enderror" value="{{ old('stok_kg') }}" min="1" required>
                            </div>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Warehouse Location</label>
                            <input type="text" name="lokasi_gudang" class="form-control @error('lokasi_gudang') is-invalid @enderror" value="{{ old('lokasi_gudang') }}" required>
                        </div>
